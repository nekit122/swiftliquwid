...
from sqlalchemy import create_engine, Column, Integer, String, Text, Boolean, DateTime, ForeignKey, JSON as SAJSON
...
app = FastAPI(title="NVTULKA 💠", version="2.0.0")
app.mount("/static", StaticFiles(directory="static"), name="static")
templates = Jinja2Templates(directory="templates")

# Создаём папки
for folder in ["static/avatars", "static/posts", "static/stories", "static/audio", "static/backgrounds", "static/voice_comments"]:
...
class User(Base):
    ...
    bg_image = Column(String(500), default="")
...
class Comment(Base):
    ...
    voice_path = Column(String(500), default="")
    created_at = Column(DateTime, default=datetime.utcnow)
...
def format_post_json(post, user, viewed_ids=None):
    files_data = [{"path": f.file_path, "type": f.file_type} for f in post.files]
    reactions = {}
    ...
    result = {
        ...
        "author": {"username": post.user.username, "avatar": post.user.avatar},
...
async def edit_profile(request: Request, db: Session = Depends(get_db), bio: str = Form(""), theme: str = Form("dark"), avatar: UploadFile = File(None), bg_image: UploadFile = File(None)):
    ...
    if avatar and avatar.filename:
        ...
        with open(f"static/avatars/{fname}", "wb") as f: f.write(await avatar.read())
        user.avatar = f"/static/avatars/{fname}"
    if bg_image and bg_image.filename:
        ext = Path(bg_image.filename).suffix
        fname = f"bg_{user.id}_{uuid.uuid4().hex[:8]}{ext}"
        with open(f"static/backgrounds/{fname}", "wb") as f: f.write(await bg_image.read())
        user.bg_image = f"/static/backgrounds/{fname}"
    db.commit()
...
async def create_post(request: Request, db: Session = Depends(get_db), content: str = Form(""), files: List[UploadFile] = File([])):
    ...
    for file in files:
        if file.filename:
            ...
            fname = f"post_{post.id}_{uuid.uuid4().hex[:8]}{ext}"
            with open(f"static/posts/{fname}", "wb") as f: f.write(await file.read())
            db.add(PostFile(post_id=post.id, file_path=f"/static/posts/{fname}", file_type=ft))
    db.commit()
...
async def delete_post(request: Request, post_id: int, db: Session = Depends(get_db)):
    ...
    if post:
        for pf in post.files:
            try: os.remove(pf.file_path.lstrip("/"))
            except: pass
        db.delete(post)
        db.commit()
    return RedirectResponse(url="/", status_code=302)
...
async def post_detail(request: Request, post_id: int, db: Session = Depends(get_db)):
    ...
    for v in post.views:
        ...
        if vu: views_data.append({"username": vu.username, "avatar": vu.avatar, "viewed_at": v.viewed_at.strftime('%d.%m.%Y %H:%M')})
...
async def add_comment(request: Request, post_id: int, db: Session = Depends(get_db), content: str = Form(""), voice: UploadFile = File(None)):
    user = get_current_user_optional(request, db)
    if not user: return RedirectResponse(url="/login", status_code=302)
    is_voice, voice_path = False, ""
    if voice and voice.filename:
        is_voice = True
        ext = Path(voice.filename).suffix or ".webm"
        fname = f"comment_{post_id}_{user.id}_{uuid.uuid4().hex[:8]}{ext}"
        with open(f"static/voice_comments/{fname}", "wb") as f: f.write(await voice.read())
        voice_path = f"/static/voice_comments/{fname}"
    comment = Comment(post_id=post_id, user_id=user.id, content=content, is_voice=is_voice, voice_path=voice_path)
    db.add(comment)
    db.commit()
    post = db.query(Post).filter(Post.id == post_id).first()
...
async def delete_comment(request: Request, post_id: int, comment_id: int, db: Session = Depends(get_db)):
    ...
    if comment:
        if comment.voice_path:
            try: os.remove(comment.voice_path.lstrip("/"))
            except: pass
        db.delete(comment)
        db.commit()
    return RedirectResponse(url=f"/post/{post_id}", status_code=302)
...
async def send_message(request: Request, chat_id: int, db: Session = Depends(get_db), content: str = Form(""), voice: UploadFile = File(None), file: UploadFile = File(None)):
    user = get_current_user_optional(request, db)
    if not user: return RedirectResponse(url="/login", status_code=302)
    is_voice, voice_path, file_path = False, "", ""
    if voice and voice.filename:
        is_voice = True
        ext = Path(voice.filename).suffix or ".webm"
        fname = f"chat_{chat_id}_{user.id}_{uuid.uuid4().hex[:8]}{ext}"
        with open(f"static/audio/{fname}", "wb") as f: f.write(await voice.read())
        voice_path = f"/static/audio/{fname}"
    if file and file.filename:
        ext = Path(file.filename).suffix
        fname = f"chat_file_{chat_id}_{user.id}_{uuid.uuid4().hex[:8]}{ext}"
        with open(f"static/posts/{fname}", "wb") as f: f.write(await file.read())
        file_path = f"/static/posts/{fname}"
    db.add(ChatMessage(chat_id=chat_id, user_id=user.id, content=content if not is_voice else "🎤 Голосовое", is_voice=is_voice, voice_path=voice_path, file_path=file_path))
    db.commit()
...
async def chat_messages_since(request: Request, chat_id: int, since_id: int = 0, db: Session = Depends(get_db)):
    ...
    return JSONResponse({"messages": [{"id": m.id, "content": m.content, "is_voice": m.is_voice, "voice_path": m.voice_path, "file_path": m.file_path, "created_at": m.created_at.isoformat(), "mine": m.user_id == user.id, "sender_name": m.user.username if m.user_id != user.id else None} for m in msgs]})
...
async def create_story(request: Request, db: Session = Depends(get_db), file: UploadFile = File(...), text_overlay: str = Form(""), text_x: float = Form(50), text_y: float = Form(50), text_scale: float = Form(1)):
    ...
    if not user: return RedirectResponse(url="/login", status_code=302)
    ext = Path(file.filename).suffix.lower()
    fname = f"story_{user.id}_{uuid.uuid4().hex[:8]}{ext}"
    with open(f"static/stories/{fname}", "wb") as f: f.write(await file.read())
    db.add(Story(user_id=user.id, file_path=f"/static/stories/{fname}", text_overlay=text_overlay, text_position={"x": text_x, "y": text_y, "scale": text_scale}, created_at=datetime.utcnow(), expires_at=datetime.utcnow() + timedelta(hours=24)))
    db.commit()
...
async def search_mention(request: Request, q: str = Query(""), db: Session = Depends(get_db)):
    ...
    for f in db.query(Follow).filter(Follow.follower_id == user.id).all():
        ...
        if u and u.is_online and q.lower() in u.username.lower():
            results.append({"username": u.username, "avatar": u.avatar})
...
async def swift_page(request: Request, db: Session = Depends(get_db)):
    ...
    # Все посты, у которых есть видео
    video_posts = db.query(Post).join(PostFile).filter(
    ...
    return templates.TemplateResponse("swift.html", {
        "request": request,
        "user": user,
        "posts": video_posts,
        "unread_notifs": unread,
        "is_new_year": is_new_year_period()
    })
...
async def api_swift(request: Request, offset: int = 0, db: Session = Depends(get_db)):
    ...
    video_posts = db.query(Post).join(PostFile).filter(
        PostFile.file_type == "video",
        Post.is_story == False
    ).order_by(Post.created_at.desc()).offset(offset).limit(10).all()

    return JSONResponse({
        "posts": [format_post_json(p, user) for p in video_posts]
    })
